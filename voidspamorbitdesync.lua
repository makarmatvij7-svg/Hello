...
local function UpdateCharacterReferences(char)
    ...
    if RootPart then lastKnownHeight = RootPart.Position.Y end
end
...
local function GetNearestPlayer()
    ...
    for _, plr in ipairs(Players:GetPlayers()) do
        if plr ~= LocalPlayer and plr.Character and plr.Character:FindFirstChild("HumanoidRootPart") then
            local dist = (plr.Character.HumanoidRootPart.Position - (RootPart and RootPart.Position or Vector3.zero)).Magnitude
...
function StartVoidSpam()
    ...
    local lastToggle, voidState = 0, false
    voidConnection = RunService.Heartbeat:Connect(function()
        ...
        if now - lastToggle > 0.028 then
            lastToggle = now; voidState = not voidState
            local pos = RootPart.Position
            RootPart.CFrame = voidState and CFrame.new(pos.X, VOID_Y, pos.Z) or CFrame.new(pos.X, lastKnownHeight + 0.5, pos.Z)
        end
    end)
    UpdateStatuses()
end
...
function StartOrbit(target)
    ...
    targetPlayer = target; isOrbiting = true
    if orbitConnection then orbitConnection:Disconnect() end
    orbitConnection = RunService.RenderStepped:Connect(function()
        if not RootPart then StopOrbit(); return end
        if not targetPlayer or not targetPlayer.Character or not targetPlayer.Character:FindFirstChild("HumanoidRootPart") then StopOrbit(); return end
        local targetRoot = targetPlayer.Character.HumanoidRootPart
        if (targetRoot.Position - RootPart.Position).Magnitude > ORBIT_MAX_DISTANCE then StopOrbit(); return end
        if Humanoid and (Humanoid.Sit or Humanoid.PlatformStand or Humanoid:GetState() == Enum.HumanoidStateType.Dead) then return end
        local angle = tick() * ORBIT_SPEED
        local offset = Vector3.new(math.cos(angle)*ORBIT_RADIUS, 10 + math.sin(angle*1.4)*6, math.sin(angle)*ORBIT_RADIUS)
        RootPart.CFrame = CFrame.new(targetRoot.Position + offset) * CFrame.Angles(0, angle*1.2, 0)
        if CAMERA_LOCK_ENABLED then
            local cam = Workspace.CurrentCamera
            if cam then cam.CFrame = CFrame.lookAt(cam.CFrame.Position, targetRoot.Position + Vector3.new(0,2.5,0)) end
        end
    end)
    UpdateStatuses()
end
...
function StopOrbit()
    isOrbiting = false; targetPlayer = nil
...
pcall(function()
    ...
    Title.Text = "EDEN-XANDER Rivals Suite v4.2"
    ...
    CreateButton("Toggle Void Spam (V)", 200, function() if isVoidSpamming then StopVoidSpam() else StartVoidSpam() end end)
    CreateButton("Toggle Desync (D)", 240, function() if isDesyncing then StopDesync() else StartDesync() end end)
    CreateButton("Toggle Orbit Nearest (O)", 280, function()
        if isOrbiting then StopOrbit() else
            local nearest = GetNearestPlayer()
            if nearest then StartOrbit(nearest) end
        end
    end)
...
getgenv().EDENXANDER_DESTROY = function()
    if heightTrackerConnection then heightTrackerConnection:Disconnect(); heightTrackerConnection = nil end
    StopVoidSpam()
    StopDesync()
    StopOrbit()
    if godModeConnection then godModeConnection:Disconnect(); godModeConnection = nil end
    if ScreenGui then ScreenGui:Destroy() end
    print("🌀 EDEN-XANDER Suite v4.2 + UI fully terminated.")
end

print("🌀 EDEN-XANDER Rivals Exploit Suite v4.2 + UI LOADED")
print("All status syncing fixed. Nearest logic centralized. UI protected.")
